"""Build conservative, additive Dataset V3 fusion masks for the two-city pilot."""
...
def main() -> None:
    ...
    parser.add_argument("--output-root", type=Path, default=Path("data/interim/labels_v3/fused_pilot"))
    parser.add_argument("--report-dir", type=Path, default=Path("metadata/dataset_v3/pilot/fusion"))
    ...
    outputs = {
        "semantic": out_dir / f"{args.city_id}_v3_pilot_semantic.tif",
        "validity": out_dir / f"{args.city_id}_v3_pilot_validity.tif",
        "provenance": out_dir / f"{args.city_id}_v3_pilot_provenance.tif",
        "support_count": out_dir / f"{args.city_id}_v3_pilot_support_count.tif",
        "conflict": out_dir / f"{args.city_id}_v3_pilot_source_conflict.tif",
    }
    report_json = args.report_dir / f"{args.city_id}_v3_pilot_fusion_qa.json"
    report_csv = args.report_dir / f"{args.city_id}_v3_pilot_class_distribution.csv"
    collisions = [p for p in [*outputs.values(), report_json, report_csv] if p.exists()]
    if collisions:
        raise FileExistsError("Refusing to overwrite existing V3 pilot outputs: " + ", ".join(map(str, collisions)))
    ...
    with rasterio.open(args.dynamic_world) as dw, rasterio.open(args.osm_semantic) as osm_sem, \
        ...
        descriptions = {
            "semantic": "dataset_v3_pilot_semantic_ignore_255",
            "validity": "dataset_v3_pilot_training_validity",
    ...
    report = {
        "schema_version": "dataset-v3-pilot-fusion-0.2",
