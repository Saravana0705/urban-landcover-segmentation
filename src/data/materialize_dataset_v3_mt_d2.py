"""Materialize 16-band V3-MT-D2 tiles from the frozen V3-MT tile set."""
...
ACCEPTED_PARENT_MANIFEST_SHA256 = (
    # LF serialization stored in the repository/Kaggle dataset.
    "8a6c3266609fbb6e060f754c9f53dd55a3b4e3681e8adafe1a88b72bda9a38b0",
    # Semantically identical CRLF serialization produced by Git on Windows.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=Path(
            "metadata/dataset_v3_mt_d2/normalization/"
    ...
    parser.add_argument(
        "--tile-root", type=Path,
        default=Path("data/processed/dataset_v3_mt_d2/tiles"),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--audit", type=Path,
        default=Path(
            "metadata/dataset_v3_mt_d2/materialization/"
            "dataset_v3_mt_d2_materialization_audit.json"
        ),
    )
...
def image_profile(source: Any, transform: Any, tile_size: int) -> dict[str, Any]:
    ...
    profile.update(
        driver="GTiff", width=tile_size, height=tile_size, count=16,
...
def validate_inputs(args: argparse.Namespace) -> tuple[pd.DataFrame, dict[str, Any]]:
    for path, label in (
        (args.parent_manifest, "frozen V3-MT manifest"),
        (args.source_qa, "V3-MT-D2 source QA"),
        (args.normalization, "V3-MT-D2 normalization"),
        (args.normalization_provenance, "normalization provenance"),
    ):
        require_file(path, label)
    actual_hash = sha256_file(args.parent_manifest)
    if actual_hash not in ACCEPTED_PARENT_MANIFEST_SHA256:
        raise RuntimeError(
            f"Frozen V3-MT manifest hash mismatch: {actual_hash}"
        )
    ...
    if source_qa.get("dataset_version") != "v3-mt-d2":
        raise RuntimeError("Source QA is not for Dataset V3-MT-D2")
    if source_qa.get("coverage_unit") != "same_day_slice_union":
        raise RuntimeError("Source QA is not based on mosaic-aware acquisitions")
    if tuple(source_qa.get("expected_band_order", [])) != EXPECTED_BANDS:
        raise RuntimeError("Source-QA band order does not match V3-MT-D2")
    ...
    if normalization.get("dataset_variant") != "v3-mt-d2":
        raise RuntimeError("Unexpected normalization dataset variant")
    ...
    if provenance.get("status") != "PASS":
        raise RuntimeError("Normalization provenance has not passed")
    qa_hashes = {
    ...
    if len(frame) != 1664 or frame["tile_id"].duplicated().any():
        raise RuntimeError("Frozen V3-MT manifest must contain 1,664 unique tiles")
...
def main() -> None:
    args = parse_args()
    require_write_scope(
        args.tile_root, Path("data/processed/dataset_v3_mt_d2"), "tile root"
    )
    require_write_scope(
        args.output_manifest, Path("metadata/dataset_v3_mt_d2"), "manifest"
    )
    require_write_scope(
        args.audit, Path("metadata/dataset_v3_mt_d2"), "audit"
    )
    ...
    if (args.output_manifest.exists() or args.audit.exists()) and not args.resume:
        raise FileExistsError("V3-MT-D2 metadata exists; inspect and use --resume")
    ...
    for city_id, city_rows in frame.groupby("city_id", sort=True):
        ...
        with rasterio.open(source_path) as temporal:
            if temporal.count != 16 or tuple(temporal.descriptions) != EXPECTED_BANDS:
            ...
            for _, row in city_rows.iterrows():
                ...
                if any(existence):
                    ...
                    image = temporal.read(
                        window=window, out_shape=(16, tile_size, tile_size),
                    ...
                    with ExitStack() as stack:
                        ...
                        atomic_write(
                            ...
                            ("dataset_v3_mt_d2_semantic_ignore_255",), tags,
                        )
                        atomic_write(
                            paths["validity"], validity,
                            mask_profile(validity_ds, transform, tile_size, False),
                            ("dataset_v3_mt_d2_training_validity",), tags,
                        )
                    written += 1
                ...
                output_row.update({
                    "parent_v3_mt_image_path": row["image_path"],
                    "parent_v3_mt_semantic_mask_path": row["semantic_mask_path"],
                    "parent_v3_mt_validity_mask_path": row["validity_mask_path"],
                    "image_path": str(paths["image"]),
                    "semantic_mask_path": str(paths["semantic"]),
                    "validity_mask_path": str(paths["validity"]),
                    "parent_v3_mt_valid_pixel_count": parent_valid,
    ...
    city_summary = output.groupby(["city_id", "city_name", "split"], as_index=False).agg(
        ...
        parent_valid_pixel_count=("parent_v3_mt_valid_pixel_count", "sum"),
    ...
    summary_path = (
        args.audit.parent / "dataset_v3_mt_d2_materialization_city_summary.csv"
    )
    ...
    audit = {
        "schema_version": "dataset-v3-mt-d2-materialization-audit-0.1",
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "status": "PASS",
        "dataset_variant": "v3-mt-d2",
        "parent_dataset": "v3-mt",
        "labels_reused_from": "v3",
        "input_channel_count": len(EXPECTED_BANDS),
        "coverage_unit": source_qa.get("coverage_unit"),
        "parent_manifest": str(args.parent_manifest),
        "parent_manifest_sha256": sha256_file(args.parent_manifest),
        "accepted_parent_manifest_sha256": list(
            ACCEPTED_PARENT_MANIFEST_SHA256
        ),
        ...
        "parent_valid_pixel_count": int(
            output["parent_v3_mt_valid_pixel_count"].sum()
        ),
    ...
    print(json.dumps({
        ...
        "next": "python -m src.quality_control.validate_dataset_v3_mt_d2_tiles",
    }, indent=2))
