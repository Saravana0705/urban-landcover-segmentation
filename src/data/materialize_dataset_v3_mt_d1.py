"""Materialize 24-band V3-MT-D1 tiles from the frozen V3-MT tile set."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--tile-root", type=Path,
        default=Path("data/processed/dataset_v3_mt_d1/tiles"),
    )
    parser.add_argument(
...
def image_profile(source: Any, transform: Any, tile_size: int) -> dict[str, Any]:
    ...
    profile.update(
        driver="GTiff", width=tile_size, height=tile_size, count=24,
...
def validate_inputs(args: argparse.Namespace) -> tuple[pd.DataFrame, dict[str, Any]]:
    for path, label in (
        ...
        (args.source_qa, "V3-MT-D1 source QA"),
        (args.normalization, "V3-MT-D1 normalization"),
    ...
    if normalization.get("dataset_variant") != "v3-mt-d1":
...
def main() -> None:
    args = parse_args()
    require_write_scope(
        args.tile_root, Path("data/processed/dataset_v3_mt_d1"), "tile root"
    )
    require_write_scope(
        args.output_manifest, Path("metadata/dataset_v3_mt_d1"), "manifest"
    )
    require_write_scope(
        args.audit, Path("metadata/dataset_v3_mt_d1"), "audit"
    )
    ...
    if (args.output_manifest.exists() or args.audit.exists()) and not args.resume:
        raise FileExistsError("V3-MT-D1 metadata exists; inspect and use --resume")
    ...
    for city_id, city_rows in frame.groupby("city_id", sort=True):
        ...
        with rasterio.open(source_path) as temporal:
            if temporal.count != 24 or tuple(temporal.descriptions) != EXPECTED_BANDS:
            ...
            for _, row in city_rows.iterrows():
                ...
                if any(existence):
                    ...
                    image = temporal.read(
                        window=window, out_shape=(24, tile_size, tile_size),
                    ...
                    with ExitStack() as stack:
                        ...
                        atomic_write(
                            ...
                            ("dataset_v3_mt_d1_semantic_ignore_255",), tags,
                        )
                        atomic_write(
                            paths["validity"], validity,
                            mask_profile(validity_ds, transform, tile_size, False),
                            ("dataset_v3_mt_d1_training_validity",), tags,
                        )
                    written += 1
    ...
    summary_path = (
        args.audit.parent / "dataset_v3_mt_d1_materialization_city_summary.csv"
    )
    ...
    audit = {
        "schema_version": "dataset-v3-mt-d1-materialization-audit-0.1",
    ...
    print(json.dumps({
        ...
        "next": "python -m src.quality_control.validate_dataset_v3_mt_d1_tiles",
    }, indent=2))
