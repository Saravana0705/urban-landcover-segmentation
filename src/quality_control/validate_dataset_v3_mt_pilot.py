"""Validate the two full-resolution Dataset V3-MT pilot rasters."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "metadata/dataset_v3_mt/pilot/"
            "dataset_v3_mt_pilot_qa.json"
        ),
    )
...
def validate_city(city: dict[str, Any], root: Path, minimum: float) -> dict[str, Any]:
    ...
    path = folder / f"{city_id}_{city_name}_S1_MT_Q1Q4_2025.tif"
    ...
    with rasterio.open(path) as dataset:
        ...
        if dataset.count != 8:
            failures.append(f"band count is {dataset.count}; expected 8")
        ...
        for index, band_name in enumerate(EXPECTED_BANDS, start=1):
...
def main() -> None:
    ...
    report = {
        "schema_version": "dataset-v3-mt-pilot-qa-0.1",
