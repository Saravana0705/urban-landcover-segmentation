"""Create the immutable metadata freeze package for Dataset V3-MT."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("metadata/dataset_v3_mt/freeze"),
    )
...
def main() -> None:
    ...
    checks = {
        ...
        "normalization_passed": (
            ...
            and normalization.get("dataset_variant") == "v3-mt"
        ...
        "all_image_paths_are_v3_mt": all(
            str(row["image_path"]).replace("/", "\\").startswith(
                "data\\processed\\dataset_v3_mt\\tiles\\"
            )
            for row in rows
        ),
    ...
    if not all(checks.values()):
        ...
        raise RuntimeError("Dataset V3-MT freeze refused: " + ", ".join(failed))
    ...
    dataset_config = {
        ...
        "input": {
            ...
            "temporal_composites": ["Q1_2025", "Q2_2025", "Q3_2025", "Q4_2025"],
        },
        "normalization": {
        ...
        "training_policy": {
            ...
            "training_selection": "identical to frozen Dataset V3",
            "validation_test_selection": "identical to frozen Dataset V3",
    ...
    freeze_record = {
        "schema_version": "dataset-v3-mt-freeze-0.1",
        "dataset_version": "v3-mt",
        "parent_label_dataset": "v3",
        "freeze_status": "FROZEN",
        ...
        "experimental_control": {
            "intervention": "replace two-band single-date SAR with eight-band quarterly SAR",
            "labels_tiles_splits_unchanged_from_v3": True,
            "paired_baseline": "unet_v3_e0_paired_50ep",
    ...
    freeze_path = args.output_dir / "dataset_v3_mt_freeze.json"
    ...
    (args.output_dir / "DATASET_V3_MT_FREEZE.md").write_text(
        "\n".join(lines) + "\n", encoding="utf-8"
    )
    print(
        json.dumps(
            {
                ...
                "split_tile_counts": split_counts,
                "input_channels": 8,
                "next_gate": "local_unet_v3_mt_integration_check",
            },
            indent=2,
        )
    )
