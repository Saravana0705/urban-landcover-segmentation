"""Create the immutable metadata freeze package for Dataset V3-MT-D2."""
...
EXPECTED_BANDS = [
    ...
    for band in (f"VV_{orbit}_Q{quarter}", f"VH_{orbit}_Q{quarter}")
]
EXPECTED_MANIFEST_SHA256 = (
    "450c0f6f5c4e69e33a3778dd9518ed30c321a5a3ed404c6bf14fd05f441e1ebb"
)
CLASSES = [
    (1, "buildings"),
    (2, "roads"),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "metadata/dataset_v3_mt_d2/qa/dataset_v3_mt_d2_tile_qa.json"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("metadata/dataset_v3_mt_d2/freeze"),
    )
...
def main() -> None:
    ...
    checks = {
        ...
        "tile_qa_passed": (
            ...
            and tile_qa.get("dataset_variant") == "v3-mt-d2"
            and tile_qa.get("input_channel_count") == 16
        ...
        "materialization_passed": (
            ...
            and materialization.get("dataset_variant") == "v3-mt-d2"
            and materialization.get("parent_dataset") == "v3-mt"
            and materialization.get("input_channel_count") == 16
            and materialization.get("coverage_unit")
            == "same_day_slice_union"
            ...
            == 1.0
            and materialization.get("de13_retained_fraction_parent_valid")
            == 1.0
        ),
        ...
        "source_qa_passed": (
            ...
            and source_qa.get("dataset_version") == "v3-mt-d2"
            and source_qa.get("city_count") == 20
            and source_qa.get("expected_band_order") == EXPECTED_BANDS
            and source_qa.get("coverage_unit") == "same_day_slice_union"
            and float(source_qa.get("minimum_valid_fraction", 0)) >= 0.98
            and source_qa.get("labels_modified") is False
            and source_qa.get("split_assignments_modified") is False
        ),
        ...
        "normalization_passed": (
            ...
            and normalization.get("dataset_variant") == "v3-mt-d2"
            and normalization.get("input_channel_count") == 16
            and normalization.get("orbit_passes")
            == ["ASCENDING", "DESCENDING"]
            and normalization.get("temporal_periods")
            == ["Q1", "Q2", "Q3", "Q4"]
        ...
        "normalization_provenance_passed": (
            ...
            and provenance.get("coverage_unit") == "same_day_slice_union"
        ...
        "all_image_paths_are_v3_mt_d2": all(
            str(row["image_path"]).replace("/", "\\").startswith(
                "data\\processed\\dataset_v3_mt_d2\\tiles\\"
            )
            for row in rows
        ),
    ...
    if not all(checks.values()):
        ...
        raise RuntimeError(
            "Dataset V3-MT-D2 freeze refused: " + ", ".join(failed)
        )
    ...
    dataset_config = {
        ...
        "input": {
            ...
            "temporal_composites": [
                f"{orbit}_Q{quarter}_2025"
                for orbit in ("ASC", "DESC")
                for quarter in range(1, 5)
            ],
            "orbit_fusion": {
                "passes": ["ASCENDING", "DESCENDING"],
                "strategy": "early_channel_concatenation",
                "coverage_unit": "same_day_slice_union",
                "daily_slice_mosaic": True,
                "quarterly_reducer": "median",
            },
        },
        "normalization": {
    ...
    freeze_record = {
        "schema_version": "dataset-v3-mt-d2-freeze-0.1",
        "dataset_version": "v3-mt-d2",
        "parent_dataset": "v3-mt",
        "parent_label_dataset": "v3",
        "freeze_status": "FROZEN",
        ...
        "experimental_control": {
            "intervention": (
                "replace each city's single-pass quarterly eight-channel "
                "stack with paired ascending/descending quarterly "
                "16-channel early fusion"
            ),
    ...
    freeze_path = args.output_dir / "dataset_v3_mt_d2_freeze.json"
    ...
    lines = [
        "# Dataset V3-MT-D2 Freeze Record",
        "",
        "- Status: **FROZEN**",
        f"- Frozen at: **{frozen_at}**",
        "- Input: **16 quarterly Sentinel-1 ascending/descending VV/VH bands**",
        "- Tiles: **1,664**",
    ...
    (args.output_dir / "DATASET_V3_MT_D2_FREEZE.md").write_text(
        "\n".join(lines) + "\n", encoding="utf-8"
    )
    print(
        json.dumps(
