"""Validate all 20 full-resolution Dataset V3-MT source rasters."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output", type=Path,
        default=Path(
            "metadata/dataset_v3_mt/source_qa/"
            "dataset_v3_mt_source_qa.json"
        ),
    )
...
def validate_city(city: dict[str, Any], root: Path, minimum: float) -> dict[str, Any]:
    ...
    with rasterio.open(path) as dataset:
        ...
        if (dataset.width, dataset.height) != (3000, 3000):
            failures.append(f"shape {dataset.width}x{dataset.height} != 3000x3000")
        if not transforms_match(dataset.transform, expected_transform):
            failures.append("transform does not match frozen city grid")
        if dataset.count != 8:
            failures.append(f"band count {dataset.count} != 8")
...
def main() -> None:
    ...
    report = {
        "schema_version": "dataset-v3-mt-source-qa-0.1",
